...
public class FastFloatBuffer {
	 ...
	 * necessary
	 */
	private static int [] mCacheIntArray = new int[0];
	...
	public FastFloatBuffer(final int pCapacity) {
		this.mByteBuffer = ByteBuffer.allocateDirect((pCapacity * BYTES_PER_FLOAT)).order(sByteOrder);
//		this.mByteBuffer = ByteBuffer.allocate((pCapacity * BYTES_PER_FLOAT)).order(sByteOrder);
	...
	public void put(final float[] data, final int length) {
		int[] ia = mCacheIntArray;
		if(ia == null || ia.length < length) {
			ia = new int[length];
			mCacheIntArray = ia;
	 ...
	 * method to put it in the bufferu
	 * 
	 ...
	 */
	public static final int[] convert(final float ... data) {
		final int length = data.length;
