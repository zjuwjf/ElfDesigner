...
public class PuppyNode extends MultiAnimateNode{ 

	public PuppyNode(ElfNode father, int copy) {
		super(father, copy);
	}
	...
	static final String [] LEFT = {"#左边","#左","#left","#zuo","左边","左","left","zuo"}; 
	static final String [] RIGHT = {"#右边","#右","#right","#you","右边","右","right","you"}; 
	static final String [] UP = {"#反","#背","#上","#up","shang","反","背","上","up","shang"}; 
	static final String [] DOWM = {"#正","#下","#down","#xia","正","下","down","xia"}; 
	
	final void setDir(final String [] dir) { 
		for(String key : dir) {
			if(setAnimateState(key)) {
				break;
			} 
		} 
	}
	...
	public void setPosition(final ElfPointf pos) { 
		final ElfPointf oldPos = getPosition();
		if(mIsInMoving) {
			final float offX = pos.x-oldPos.x;
			final float offY = pos.y-oldPos.y;
			if(offX != 0 || offY != 0) {
	...
	public static interface IOnIndexListener{
		public void onPassIndex(final int index, boolean isAddIndex);
	} 
	...
	final static boolean isOnReversWay(final float rate) {
